...
const GlassCard = ({ children, className = "" }) => (
    ...
    bg-[#0f1116] border border-yellow-500/20 shadow-2xl
    rounded-3xl p-6 
    ${className}
  `}>
...
const ContentEditor = ({ courseData, setCourseData, onBack, onFinalSubmit }) => {
    ...
    const tempVideoFile = tempVideoFiles[activeClassIndex];
    const tempAudioFile = tempAudioFiles[activeClassIndex];
    ...
    const handleSaveClass = async () => {
        ...
        try {
            ...
            setCourseData({ ...courseData, classes: newClasses });
            setTempVideoFiles(prev => { const next = { ...prev }; delete next[targetIdx]; return next; });
            setTempAudioFiles(prev => { const next = { ...prev }; delete next[targetIdx]; return next; });
    ...
    return (
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 h-full">
            ...
            <div className="xl:col-span-3 space-y-4">
                <GlassCard className="h-fit sticky top-32 border-yellow-500/20">
                                ...
                                <motion.div
                                    ...
                                    className={`
                                        ...
                                        ${idx === activeClassIndex
                                            ...
                                            : 'bg-[#18181b] border-yellow-500/20 text-[#D1F2EB]/70 hover:border-gray-600 hover:text-gray-200'
                                        }
                                    `}
                                >
                                    <div className="flex justify-between items-center relative z-10">
                                        <div className="flex items-center gap-3">
                                            <span className={`text-xs font-bold w-6 h-6 flex items-center justify-center rounded-lg ${idx === activeClassIndex ? 'bg-[#0B6E4F] text-[#D1F2EB]' : 'bg-[#0B6E4F] text-yellow-400'}`}>
                                                {idx + 1}
                                            </span>
                    ...
                    <div className="space-y-3 pt-6 border-t border-yellow-500/20">
                        ...
                        <motion.button
                            whileHover={{ x: -2 }}
                            onClick={onBack}
                            className="w-full py-3 text-sm text-yellow-400 hover:text-[#D1F2EB] border border-yellow-500/30 hover:border-gray-500 rounded-xl transition-all flex items-center justify-center gap-2"
                        >
                            <ArrowLeft size={16} />
                            Go Back
                        </motion.button>
            ...
            <div className="xl:col-span-9">
                <GlassCard className="min-h-full border-yellow-500/20">
                    {/* Header */}
                        ...
                        <AnimatePresence>
                            {currentUpload.uploading && (
                                <motion.div
                                    ...
                                    exit={{ opacity: 0, x: 20 }}
                                    className="w-full md:w-72 bg-[#0a0a0a] rounded-xl border border-yellow-500/30 p-4 shadow-xl"
                                >
                                    ...
                                    <div className="w-full h-3 bg-[#0B6E4F] rounded-full overflow-hidden border border-yellow-500/30/50">
                                        <motion.div
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                        {/* Video Upload Card */}
                        <div className="bg-[#18181b] p-6 rounded-2xl border border-yellow-500/20 relative group hover:border-[#0B6E4F]/30 transition-all">
                            <label className="flex flex-col h-full cursor-pointer">
                                ...
                                <div className="relative w-full h-48 border-2 border-dashed border-yellow-500/30 rounded-xl flex flex-col items-center justify-center bg-black/20 group-hover:bg-[#0B6E4F]/5 group-hover:border-[#0B6E4F]/50 transition-all overflow-hidden">
                                    {tempVideoFile ? (
                                        <div className="text-center p-4">
                                            ...
                                            <p className="text-xs text-yellow-400 mt-1">Ready to upload</p>
                                        </div>
                                    ) : currentClass.video ? (
                                        ...
                                        <>
                                            ...
                                            <span className="text-sm text-yellow-400 font-medium group-hover:text-purple-300">
                                ...
                                <input
                                    ...
                                    accept="video/*"
                                    className="hidden"
                                    onChange={e => { if (e.target.files[0]) { setTempVideoFiles(prev => ({ ...prev, [activeClassIndex]: e.target.files[0] })) } }}
                                />
                            </label>
                        </div>
                        ...
                        <div className="bg-[#18181b] p-6 rounded-2xl border border-yellow-500/20 relative group hover:border-[#0B6E4F]/30 transition-all">
                            <label className="flex flex-col h-full cursor-pointer">
                                ...
                                <div className="relative w-full h-48 border-2 border-dashed border-yellow-500/30 rounded-xl flex flex-col items-center justify-center bg-black/20 group-hover:bg-[#0B6E4F]/5 group-hover:border-[#0B6E4F]/50 transition-all">
                                    {tempAudioFile ? (
                                        <div className="text-center p-4">
                                ...
                                <input
                                    ...
                                    accept="audio/*"
                                    className="hidden"
                                    onChange={e => { if (e.target.files[0]) { setTempAudioFiles(prev => ({ ...prev, [activeClassIndex]: e.target.files[0] })) } }}
                                />
                            </label>
                        </div>
                    </div>
                    ...
                    <div className="mb-8">
                        ...
                        <textarea
                            className="w-full bg-[#18181b] border border-yellow-500/20 rounded-xl p-5 text-[#D1F2EB]/90 focus:outline-none focus:border-[#0B6E4F] focus:ring-1 focus:ring-[#0B6E4F] transition-all min-h-[150px] resize-y placeholder:text-yellow-100/40"
                    ...
                    {/* Save Action */}
                    <div className="flex justify-end border-b border-yellow-500/20 pb-8 mb-8">
                        <motion.button
                            whileHover={{ scale: 1.02 }}
                            whileTap={{ scale: 0.98 }}
                            ...
                            <AnimatePresence>
                                {currentClass.mcq.map((q, i) => (
                                    <motion.div
                                        ...
                                        className="bg-[#18181b] p-6 rounded-xl border border-yellow-500/20 hover:border-yellow-500/30 transition-colors relative"
                                    >
                                        <button
                                            onClick={() => removeMCQ(i)}
                                            className="absolute top-4 right-4 text-yellow-200 hover:text-red-400 transition-colors"
                                        >
                                            <X size={18} />
                                        </button>

                                        <div className="flex items-center gap-4 mb-4">
                                            <span className="text-yellow-400 font-mono text-sm font-bold">0{i + 1}.</span>
                                            <input
                                                className="flex-1 bg-transparent border-b border-yellow-500/30 focus:border-[#0B6E4F] focus:outline-none text-[#D1F2EB] font-medium py-1 transition-colors placeholder:text-yellow-100/40"
                                                    ...
                                                    <div key={optIdx} className="flex gap-3 items-center group">
                                                        ...
                                                        <input
                                                            className={`
                                                                ...
                                                                ${isSelected ? 'border-green-500/50 bg-green-900/10' : 'border-yellow-500/30 focus:border-[#0B6E4F]'}
                                                            `}
